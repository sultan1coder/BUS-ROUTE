...
export class DriverAppService {
  ...
  static async startTrip(
    ...
    if (!driver || !driver.bus || driver.bus.routes.length === 0) {
      throw new CustomError("You don't have access to this route", 403);
    }
    ...
    if (activeTrip) {
      throw new CustomError("You already have an active trip", 400);
    }
  ...
  static async updateTrip(
    ...
    if (!trip) {
      throw new CustomError("Trip not found or access denied", 404);
    }
    ...
    if (currentLocation) {
      await GPSTrackingService.recordLocation({
  ...
  static async getCurrentTrip(driverId: string): Promise<any> {
    const trip = await prisma.trip.findFirst({
      where: {
        driver: {
          userId: driverId,
  ...
  static async getTripNavigation(
    driverId: string,
  ...
  static async recordStudentPickup(
    driverId: string,
    tripId: string,
  ...
  static async recordStudentDrop(
    driverId: string,
    tripId: string,
    dropData: StudentDropData
  ): Promise<any> {
  ...
  static async getStudentManifest(
    ...
    const attendanceMap = new Map(
      attendance.map((att) => [att.studentId, att])
    );
  ...
  static async updateDriverLocation(
    ...
    if (!driver || !driver.bus) {
      throw new CustomError("Driver or bus not found", 404);
    }
    ...
    await SafetyService.checkGeofenceViolation(
      driver.bus.id,
      location.latitude,
      location.longitude
    );
  ...
  static async getDriverDashboard(driverId: string): Promise<DriverDashboard> {
  ...
  static async getDriverTripHistory(
    driverId: string,
    page: number = 1,
    limit: number = 20,
    filters?: {
  ...
  static async triggerDriverEmergency(
    driverId: string,
    emergencyData: {
    ...
    if (!driver || !driver.bus) {
      throw new CustomError("Driver bus information not found", 404);
    }

    // Trigger emergency alert
    const alert = await SafetyService.triggerSOSAlert(driverId, {
      busId: driver.bus.id,
      driverId: driver.id,
  ...
  static async getDriverReport(
    ...
    const [tripCount, totalStudents, totalDistance, completedTrips] =
      await Promise.all([
        // Total trips
        prisma.trip.count({
          where: {
  ...
  static async getDriverRoutes(driverId: string): Promise<any[]> {
  ...
  private static async getDriverRecentActivity(
    driverId: string,
    limit: number
  ): Promise<any[]> {
  ...
  private static async getDriverTripStats(driverId: string): Promise<any> {
    const today = new Date();
    today.setHours(0, 0, 0, 0);

    const [totalTripsToday, totalStudentsTransported, totalDistance] =
      await Promise.all([
        prisma.trip.count({
          where: {
            driver: { userId: driverId },
  ...
  private static calculateNextStop(
    ...
    if (!currentLocation) return stops[0];
  ...
  private static emitTripEvent(event: string, data: any): void {
    io.emit(event, data);

    // Also emit to specific rooms
    if (data.tripId) {
      io.to(`trip_${data.tripId}`).emit(event, data);
    }
    if (data.driverId) {
  ...
  private static emitDriverEvent(event: string, data: any): void {
    io.emit(event, data);
